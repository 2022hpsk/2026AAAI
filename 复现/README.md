...
## 进展 2026-06-10 ④：A-MEM + HippoRAG SocialMem 全量 + Mem0 失效深度归因

**A-MEM / HippoRAG SocialMem 全量出数（四基线齐，judge acc%）**：

| SocialMem 总体 | BM25 | Mem0 | A-MEM | HippoRAG |
|---|---|---|---|---|
| 1031 题 | 28.6 | 13.7 | **48.1** | **59.0** |

- ⚡ **关键发现**：SocialMem 上**图结构记忆(HippoRAG/A-MEM)反超 BM25 与 Mem0**——和 GroupMem/EverMem 上"BM25 碾压"相反。Mem0 三场全垫底。
- 命令：
  - `PYTHONPATH=. ./venv/bin/python run_amem.py --benchmark socialmem --domain all --max-messages 100000 --batch-size 64 --ingest-workers 16 --qa-workers 16`
  - `conda run -n hippo env PYTHONPATH=. python run_hipporag.py --benchmark socialmem --domain all --max-passages 1000 --qa-workers 8`
  - 算指标：`compute_metrics.py --benchmark socialmem --baseline {amem,hipporag}`
- 持久化：A-MEM 加 pickle 落盘 `.amem_store/`（复用验证 OK）；HippoRAG 自带 `.hipporag_store/`；并发竞态用 `evo_threshold=1e9`(禁入库期 consolidate) 修掉。
- 分类详表 + 成本实测见 `实验结果.md §1/§3/§7`。

**Mem0 失效深度归因（`Mem0失效分析.md` + `analyze_mem0_fail.py`）**：
- 方法：对 **BM25对/Mem0错** 的题，**重建两边实际检索内容**（BM25→原始消息；Mem0→查持久化 chroma 里被 LLM 抽取后的记忆），逐例对照（全程不调 API、免费）。
- **5+1 大根因**：①精确字面值被摘要抹平 ②属性-实体绑定断裂 ③说话人/归属丢失 ④言语行为/立场被丢 ⑤关系边缺失+显著性偏置 ⑥时间更新不追踪。
- **结论**：Mem0 失败不在检索，在**入库 LLM 摘要把「精确字面值 / 说话人归属 / 人际关系」压没了**；BM25 靠逐字原文+全元数据不丢、HippoRAG/A-MEM 靠实体-关系图/笔记链接保 who-said-what 才不崩 → 直接、可量化地论证 speaker-grounded 结构化记忆。
- PPT：`汇报slides-build/gen_v2.js` 更新（17 页）——page12 四基线总表、page13 SocialMem 四基线分类表、新增 page14 Mem0 失效根因。

---

## 进展 2026-06-10 ⑤：SocialMem 口径审查（统一检索预算 + MeanQ/MeanN）

**问题**：HippoRAG 59 / A-MEM 48 疑似超过论文方法——是真高还是口径问题？

**审查**（`SocialMem口径审查.md` + `rejudge_paper_rubric.py` + `analyze_mem0_fail2.py`）：
1. **三个论文检索预算都是 top-10 单条**（GroupMem BM25 top-10 消息 / SocialMem naive_rag+mem0 top-10 turn+记忆 / EverMem adapter top-10）。HippoRAG 之前 top-10 passage=**80 条原文**=越界。
2. **MeanQ/MeanN**：论文每题 0–1 部分给分(开放题 LLM judge，错归属≤0.3)；MeanQ=按题平均，MeanN=按 43 网络各自均分再平均(±bootstrap CI)。我们之前用二值 acc(DeepSeek judge)，偏宽松、不可直接比。

**公平预算 top-10 终表（该报的口径）**：
...
→ **真实排序 A-MEM > BM25 > HippoRAG > Mem0**。
- 受控实验B（预算）：`run_baseline --top-k 80 --results-dir results_budget80` → BM25 28.6→**57.3**≈HippoRAG@80 59；`run_hipporag --top-k 1 --results-dir results_k1` → HippoRAG 暴跌 59→**22.9**<BM25@10 → 图谱没加分，59 全是预算。
- **A-MEM 同预算大幅超 BM25 = 结构化记忆有效的真证据**；Mem0 全口径垫底=核心 motivation；HippoRAG 数字不可用。

**新增脚本**：`rejudge_paper_rubric.py`（论文浮点 rubric 重评出 MeanQ/MeanN，支持 `--results-dir/--tag`）、`analyze_mem0_fail2.py`（判抽取丢失 vs 检索未命中）。
**Mem0 失效细判**（`Mem0失效分析.md` 详尽版）：5 例区分根因——几乎没有"事实完好进库纯败检索"，答案关键信息多在**入库摘要阶段**就被丢/压平（字面值/言语行为/关系温度/时序）→ 更大 top-k 也救不了。
**PPT**：`gen_v2.js` 18 页，page15 改为"统一预算后真实排序 + 预算敏感性"。
