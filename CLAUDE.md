...
- **A-MEM + HippoRAG ✅ SocialMem 全量 + 口径审查（2026-06-10）**：
  - ⚠️ **先前"HippoRAG 59 反超 BM25"是检索预算假象**，已纠正。审查见 `复现/SocialMem口径审查.md`。
  - **三个论文检索预算都是 top-10 单条**（GroupMem BM25 top-10 消息 / SocialMem naive_rag+mem0 top-10 turn+记忆 / EverMem 各 adapter top-10）→ 统一到 top-10 才公平。HippoRAG 之前 top-10 passage=**80 条原文**(≈半篇对话)是越界异类。
  - **公平预算 top-10 真实排序：A-MEM(48.1) > BM25(28.6) > HippoRAG(22.9) > Mem0(13.7)**（acc）。MeanQ：0.345/0.221/0.184/0.093。
    - 受控实验B：BM25@10→@80(80条) acc 28.6→57.3≈HippoRAG@80 59；HippoRAG 压到公平预算(1passage≈8条)暴跌到 22.9<BM25@10 → 图谱没加分，59 全是预算。
    - 受控实验A：用论文 0–1 浮点 rubric(部分给分,错归属≤0.3)重评，我们二值 acc 偏宽松。
  - ✅ **能立的结论**：Mem0 全口径垫底=核心 motivation；**A-MEM 同预算超 BM25 = "结构化记忆有效"的真证据**（支撑 SpeakerMem-R1）；HippoRAG 数字不可用。
  - **指标口径**：SocialMemBench 论文用 **MeanQ(按题平均)/MeanN(按43网络平均±bootstrapCI)**，开放题 0–1 部分给分(judge=gemini/gpt-4o-mini)；我们用二值 acc(DeepSeek judge)，不可直接比绝对值。脚本 `rejudge_paper_rubric.py` 可出 MeanQ/MeanN。
  - 持久化：A-MEM pickle `.amem_store/`(复用OK)；HippoRAG 自带 `.hipporag_store/`；并发竞态用 `evo_threshold=1e9` 修掉。A-MEM 非截断成本实测(非思考) 302note ¥0.38、全量~¥417。
